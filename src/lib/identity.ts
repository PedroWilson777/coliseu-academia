...
function phoneSearchVariants(phone: string): string[] {
  ...
  for (const v of [withDDI, withoutDDI]) {
    ...
    if (localNumber.length === 8) { variants.add(digits.slice(0, areaOffset) + '9' + localNumber); }
    if (localNumber.length === 9 && localNumber.startsWith('9')) { variants.add(digits.slice(0, areaOffset) + localNumber.slice(1)); }
  }
...
export async function identifyByPhone(phone: string, pushName?: string): Promise<Identity> {
  ...
  const student = await prisma.student.findFirst({ where: { phone: { in: variants } } });
  if (student) {
    return { type: 'STUDENT', student: { id: student.id, name: student.name, phone: student.phone } };
  }
  let lead = await prisma.lead.findFirst({ where: { phone: { in: variants } } });
  if (!lead) {
    const normalizedPhone = normalizePhone(phone);
    lead = await prisma.lead.create({ data: { phone: normalizedPhone, name: pushName } });
    console.log('Novo lead: ' + normalizedPhone + ' (' + (pushName || 'sem nome') + ')');
  }
  return { type: 'LEAD', lead: { id: lead.id, name: lead.name, phone: lead.phone } };
}
...
export async function getOrCreateConversation(identity: Identity) {
  ...
  let conversation = await prisma.conversation.findFirst({ where, orderBy: { lastMessageAt: 'desc' } });
  if (!conversation) {
