...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (signInError) { setError('Usuário ou senha incorretos.'); return; }
    router.push('/');
    router.refresh();
  };
  ...
  return (
    <div className="min-h-screen w-full grid place-items-center p-6"
      style={{ background: `radial-gradient(circle at 50% 30%, var(--accent-soft), transparent 60%), var(--bg)` }}>
      <div className="w-full max-w-md text-center">
        <div className="mb-8 flex justify-center">
          <div className="w-20 h-20 rounded-full grid place-items-center"
            style={{ background: 'linear-gradient(135deg, var(--accent), #8b1820)', boxShadow: '0 0 0 1px var(--accent), 0 0 40px var(--accent-glow)' }}>
            <span className="font-display text-4xl text-white">C</span>
          </div>
        </div>
        <h1 className="font-display text-6xl mb-3 brand-glow" style={{ color: 'var(--text)', letterSpacing: '0.05em' }}>COLISEU</h1>
        <p className="text-sm uppercase tracking-[0.2em] mb-12" style={{ color: 'var(--text-3)' }}>Painel de Gestão</p>
        <div className="rounded-2xl p-8"
          style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: '0 0 60px var(--accent-soft)' }}>
          <h2 className="font-display text-2xl mb-2">Acesso restrito</h2>
          <p className="text-sm mb-6" style={{ color: 'var(--text-2)' }}>Entre com seu usuário e senha</p>
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <input type="text" value={username} onChange={e => setUsername(e.target.value)}
              placeholder="Usuário (ex: jordancoliseu)" required autoFocus autoComplete="username" disabled={loading}
              className="w-full px-4 py-3 rounded-xl text-sm"
              style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text)', outline: 'none' }} />
            <input type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder="Senha" required autoComplete="current-password" disabled={loading}
              className="w-full px-4 py-3 rounded-xl text-sm"
              style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text)', outline: 'none' }} />
            <button type="submit" disabled={loading || !username.trim() || !password}
              className="w-full px-6 py-3 rounded-xl font-medium transition-all mt-1"
              style={{ background: loading ? 'var(--surface-2)' : 'var(--accent)', color: 'white',
          ...
          <div className="mt-6 text-xs" style={{ color: 'var(--text-3)' }}>Apenas equipe autorizada tem acesso</div>
        </div>
        <p className="mt-8 text-xs" style={{ color: 'var(--text-4)' }}>🏛️ Coliseu Academia · Teixeira de Freitas - BA</p>
      </div>
    </div>
  );
}'use client';
